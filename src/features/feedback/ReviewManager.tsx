...
export default function ReviewManager() {
  ...
  async function confirmDelete() {
    ...
    } catch (error) { toast.error(safeError(error, t("Unable to delete this review."))); }
  }
  ...
  return (
    <div className="space-y-7 font-google-sans">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-[#003377] dark:text-[#FFC83D] md:text-3xl">{t("Reviews & Feedback")}</h1>
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground font-normal">{t("Review and manage user suggestions, bug reports, complaints, compliments, and general feedback.")}</p>
      </header>
      ...
      <Card className="rounded-2xl shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg font-semibold md:text-xl">{t("Reviews")}</CardTitle>
                ...
                <Table>
                  <TableHeader className="bg-muted/40">
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="min-w-72 text-base font-semibold">{t("Review")}</TableHead>
                      <TableHead className="min-w-48 text-base font-semibold">{t("User")}</TableHead>
                      <TableHead className="min-w-36 text-base font-semibold">{t("Type")}</TableHead>
                      <TableHead className="text-base font-semibold">{t("Rating")}</TableHead>
                      <TableHead className="min-w-32 text-base font-semibold">{t("Status")}</TableHead>
                      <TableHead className="min-w-36 text-base font-semibold">{t("Created")}</TableHead>
      ...
      <AlertDialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>“{deleteTarget?.title}” {t("will be removed from review management. This action cannot be undone.")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteState.isLoading} onClick={() => setDeleteTarget(null)}>{t("Cancel")}</AlertDialogCancel>
            <AlertDialogAction variant="destructive" disabled={deleteState.isLoading} onClick={confirmDelete}>
              {deleteState.isLoading ? <RefreshCw className="mr-2 size-4 animate-spin" /> : <Trash2 className="mr-2 size-4" />}
...
function StatCard({ icon: Icon, label, value, helper }: { icon: typeof Star; label: string; value: React.ReactNode; helper: string }) {
  return (
    <Card className="rounded-2xl shadow-sm">
      <CardContent className="flex gap-4 p-5 sm:p-6">
        <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#003377]/10 text-[#003377] dark:text-[#FEDB55]"><Icon className="size-5" /></span>
        <div>
...
function TypeBadge({ type }: { type: ReviewType }) {
  ...
  return <Badge variant="outline" className={`gap-1.5 text-sm ${meta.classes}`}><Icon className="size-3.5" />{t(meta.label)}</Badge>;
}

function StatusBadge({ status }: { status: ReviewStatus }) {
  const { t } = useAdminI18n();
  const meta = STATUS_META[status];
  return <Badge variant="outline" className={`text-sm ${meta.classes}`}>{t(meta.label)}</Badge>;
}
...
function Rating({ value, detailed = false }: { value: number | null; detailed?: boolean }) {
  const rating = Math.max(0, Math.min(5, value ?? 0));
  return (
    <div className="flex items-center gap-2">
      <div className="flex" aria-label={`${value ?? "No"} rating out of 5`}>
        {Array.from({ length: 5 }).map((_, index) => (
          <Star key={index} className={`size-4 ${index < Math.round(rating) ? "fill-amber-400 text-amber-400" : "text-muted-foreground/35"}`} />
        ))}
      </div>
      <span className={detailed ? "text-base font-medium" : "text-base font-semibold"}>{value == null ? "—" : detailed ? `${value} / 5` : value}</span>
    </div>
  );
}

function ReviewRow({ review, user, onView, onDelete }: { review: Review; user?: AdminUser; onView: () => void; onDelete: () => void }) {
  const { t } = useAdminI18n();
  return (
    <TableRow className="cursor-pointer" onClick={onView}>
      <TableCell className="py-4">
        <p className="max-w-72 truncate text-base font-semibold">{review.title}</p>
        <p className="mt-1 max-w-72 truncate text-sm text-muted-foreground font-normal">{review.description}</p>
      </TableCell>
      <TableCell className="py-4"><UserIdentity user={user} /></TableCell>
      <TableCell className="py-4"><TypeBadge type={review.reviewType} /></TableCell>
      <TableCell className="py-4"><Rating value={review.overallRating} /></TableCell>
      <TableCell className="py-4"><StatusBadge status={review.reviewStatus} /></TableCell>
      <TableCell className="py-4 text-sm text-muted-foreground font-normal">
        <Tooltip><TooltipTrigger>{dateText(review.createdAt)}</TooltipTrigger><TooltipContent>{dateText(review.createdAt, true)}</TooltipContent></Tooltip>
      </TableCell>
      <TableCell className="py-4 text-right" onClick={(event) => event.stopPropagation()}>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" aria-label={`${review.title} actions`}>
              <MoreHorizontal className="size-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent>
            <DropdownMenuItem onClick={onView}><Eye className="size-4" />{t("View Details")}</DropdownMenuItem>
            <DropdownMenuItem onClick={onView}><CircleDot className="size-4" />{t("Update Status")}</DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem destructive onClick={onDelete}><Trash2 className="size-4" />{t("Delete Review")}</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </TableCell>
    </TableRow>
...
function ReviewDetailSheet({ reviewId, open, onOpenChange, usersById, onDelete }: { reviewId: string | null; open: boolean; onOpenChange: (open: boolean) => void; usersById: Map<string, AdminUser>; onDelete: (review: Review) => void }) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="max-w-[620px]" onClose={() => onOpenChange(false)}>
        <SheetHeader><SheetTitle>{t("Review Details")}</SheetTitle></SheetHeader>
        {detailQuery.isLoading ? (
          <SheetBody><DetailSkeleton /></SheetBody>
        ) : detailQuery.isError || !detailQuery.data ? (
          <SheetBody><ErrorState onRetry={() => detailQuery.refetch()} /></SheetBody>
        ) : (
          <ReviewDetail key={`${detailQuery.data.id}-${detailQuery.data.updatedAt}`} review={detailQuery.data} submitter={usersById.get(detailQuery.data.userId)} reviewer={detailQuery.data.reviewedBy ? usersById.get(detailQuery.data.reviewedBy) : undefined} onDelete={() => onDelete(detailQuery.data)} />
        )}
      </SheetContent>
    </Sheet>
  );
...
function ReviewDetail({ review, submitter, reviewer, onDelete }: { review: Review; submitter?: AdminUser; reviewer?: AdminUser; onDelete: () => void }) {
  ...
  async function save() {
    try {
      await update({ id: review.id, reviewStatus, latestReviewNote: note }).unwrap();
    ...
    <>
      <SheetBody>
        <section>
          ...
          <div className="mt-5">
            <p className="mb-2 text-sm text-muted-foreground font-normal">{t("Submitted by")}</p>
        ...
        <section>
          ...
          <p className="mt-2 whitespace-pre-wrap text-base leading-7 text-muted-foreground font-normal">{review.description}</p>
        </section>
        <Separator />
        <section>
          ...
          <div className="mt-3 space-y-3 rounded-2xl bg-muted/40 p-4">
            ...
            <RatingRow label={t("Performance")} value={review.performanceRating} />
            <RatingRow label={t("Ease of Use")} value={review.easeOfUseRating} />
        ...
        <section>
          ...
          {review.screenshotUrl && !imageFailed ? (
            <div className="mt-3">
              <button type="button" onClick={() => setScreenshotOpen(true)} className="relative h-56 w-full overflow-hidden rounded-2xl border bg-muted/30 p-2">
                <Image unoptimized fill sizes="(max-width: 640px) 100vw, 580px" src={review.screenshotUrl} alt={`Screenshot for ${review.title}`} onError={() => setImageFailed(true)} className="object-contain p-2" />
              </button>
              <Button variant="outline" className="mt-3 text-base font-medium" onClick={() => setScreenshotOpen(true)}>
                <ImageIcon className="mr-2 size-4" />{t("View Full Screenshot")}
              </Button>
            </div>
          ) : (
            <div className="mt-3 flex items-center gap-3 rounded-2xl border border-dashed p-5 text-base text-muted-foreground">
              <ImageIcon className="size-5" />{t("No screenshot attached")}
            </div>
          )}
        </section>
        <Separator />
        <section>
          ...
          {review.latestReviewNote && (
            <div className="mt-3 rounded-xl bg-muted/50 p-3">
              ...
              <p className="mt-1 text-base text-muted-foreground font-normal">{review.latestReviewNote}</p>
            </div>
          )}
          <div className="mt-4">
            <label className="text-base font-medium">{t("Status")}</label>
            <div className="mt-2">
              <ReviewSelect value={reviewStatus} options={{ PENDING: t("Pending"), IN_REVIEW: t("In Review"), RESOLVED: t("Resolved"), CLOSED: t("Closed") }} onChange={(value) => setReviewStatus(value as ReviewStatus)} />
            </div>
          </div>
          <div className="mt-4">
            <label htmlFor="review-note" className="text-base font-medium">{t("Review Note")}</label>
            <textarea id="review-note" value={note} onChange={(event) => setNote(event.target.value)} rows={5} placeholder={t("Add a note about how this review is being handled...")} className="mt-2 w-full resize-y rounded-xl border border-input bg-background px-3 py-2.5 text-base font-normal outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <Button className="mt-4 bg-[#FEDB55] text-base font-medium text-[#003377] hover:bg-[#f0ca43]" disabled={updateState.isLoading} onClick={save}>
            {updateState.isLoading ? <RefreshCw className="mr-2 size-4 animate-spin" /> : <Save className="mr-2 size-4" />}
        ...
        <section>
          ...
          <div className="mt-2 divide-y">
            <InfoRow label={t("Reviewed By")} value={review.reviewedBy ? displayName(reviewer) : t("Not reviewed yet")} />
            <InfoRow label={t("Reviewed At")} value={dateText(review.reviewedAt, true)} />
            <InfoRow label={t("Created At")} value={dateText(review.createdAt, true)} />
            <InfoRow label={t("Updated At")} value={dateText(review.updatedAt, true)} />
          </div>
        </section>
      </SheetBody>
      <SheetFooter className="justify-between">
        <Button variant="ghost" className="text-base font-medium text-destructive hover:text-destructive" onClick={onDelete}>
          <Trash2 className="mr-2 size-4" />{t("Delete Review")}
        </Button>
      </SheetFooter>
      <ScreenshotDialog open={screenshotOpen} onOpenChange={setScreenshotOpen} url={review.screenshotUrl} title={review.title} />
    </>
  );
}
...
function ScreenshotDialog({ open, onOpenChange, url, title }: { open: boolean; onOpenChange: (open: boolean) => void; url: string | null; title: string }) {
  const { t } = useAdminI18n();
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-5xl overflow-auto" onClose={() => onOpenChange(false)}>
        <DialogHeader>
          <DialogTitle>{t("Review Screenshot")}</DialogTitle>
          <DialogDescription className="text-base font-normal">{t("Screenshot attached to")} “{title}”.</DialogDescription>
        </DialogHeader>
        {url && (
          <div className="relative min-h-72 w-full rounded-xl bg-muted/40">
            <Image unoptimized width={1600} height={1200} src={url} alt={`Full screenshot for ${title}`} className="max-h-[75vh] h-auto w-full object-contain p-3" />
          </div>
        )}
      </DialogContent>
    </Dialog>
...
function EmptyState({ filtered, onReset }: { filtered: boolean; onReset: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed py-14 text-center">
      ...
      <p className="text-base font-semibold">{filtered ? t("No reviews found") : t("No reviews yet")}</p>
      <p className="text-xs text-muted-foreground font-normal">{filtered ? t("No reviews match the selected filters.") : t("User feedback and reports will appear here when submitted.")}</p>
      {filtered && (
