      ...
      <DialogContent className="max-w-[400px] p-8 rounded-[28px] font-google-sans">
        <div className="flex items-center justify-between mb-6">
          <DialogTitle className="text-xl font-bold text-[#003377] dark:text-white font-google-sans">
            {dict.dashboard.inactiveModalTitle}
          </DialogTitle>
          <button
            type="button"
        ...
        <div className="space-y-4 mb-8">
          {isLoading ? (
            ...
            <>
              <StatRow label={dict.dashboard.totalLabel} value={data?.totalInActive ?? 2} />
              <StatRow label={dict.dashboard.moreThan30Days} value={data?.moreThan30Days ?? 2} />
              <StatRow label={dict.dashboard.moreThan90Days} value={data?.moreThan90Days ?? 1} />
              <StatRow label={dict.dashboard.pendingClose} value={data?.pendingClose ?? 0} />
            </>
          )}
        </div>
...
function StatRow({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
      ...
      <span className="text-[#003377] dark:text-[#FFC83D] font-bold text-lg">
