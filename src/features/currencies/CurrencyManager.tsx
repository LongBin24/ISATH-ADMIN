...
type CurrencyAction = { currency: CurrencyItem; action: "activate" | "deactivate" } | null;
const PAGE_SIZE = 10;
...
function StatCard({ icon: Icon, label, value, helper }: { icon: typeof Coins; label: string; value: React.ReactNode; helper: string }) {
  return (
...
export default function CurrencyManager() {
  ...
  const [synchronize, { isLoading: isSyncing }] = useSynchronizeCurrenciesMutation();
  const [activate, { isLoading: isActivating }] = useActivateCurrencyMutation();
  const [deactivate, { isLoading: isDeactivating }] = useDeactivateCurrencyMutation();
  ...
  const filteredCurrencies = useMemo(() => {
    ...
    return currencies.filter((currency) => {
      const matchesQuery = !normalized || [currency.code, currency.name, currency.symbol].some((value) => value?.toLowerCase().includes(normalized));
      const matchesStatus = statusFilter === "ALL" || (statusFilter === "ACTIVE" ? currency.active : !currency.active);
  ...
  const paginatedCurrencies = useMemo(
    () => filteredCurrencies.slice((visiblePage - 1) * PAGE_SIZE, visiblePage * PAGE_SIZE),
    [filteredCurrencies, visiblePage],
  );
  const firstVisibleCurrency = filteredCurrencies.length === 0 ? 0 : (visiblePage - 1) * PAGE_SIZE + 1;
  const lastVisibleCurrency = Math.min(visiblePage * PAGE_SIZE, filteredCurrencies.length);
  ...
  async function handleSynchronize() {
    try {
      ...
      if (status === "FAILED") toast.error(result.errorMessage || "Currency synchronization failed.");
      else toast.success(status === "STARTED" ? "Synchronization started successfully." : status === "SUCCESS" || status === "COMPLETED" ? "Synchronization completed successfully." : "Synchronization request accepted.");
    } catch {
  ...
  async function confirmAction() {
    ...
    try {
      const updated = action === "activate" ? await activate(currency.code).unwrap() : await deactivate(currency.code).unwrap();
      if (selectedCurrency?.code === updated.code) setSelectedCurrency(updated);
      toast.success(`${currency.code} ${action === "activate" ? "activated" : "deactivated"} successfully.`);
  ...
  return (
    <div className="space-y-7 font-google-sans">
      ...
      <ProviderCard provider={providerQuery.data} isLoading={providerQuery.isLoading} isError={providerQuery.isError} onRetry={() => providerQuery.refetch()} onSynchronize={handleSynchronize} isSyncing={isSyncing} />
          ...
          {!currenciesQuery.isLoading && !currenciesQuery.isError && filteredCurrencies.length > 0 && (
            <div className="flex flex-col gap-3 border-t border-border pt-4 text-base lg:flex-row lg:items-center lg:justify-between">
              ...
              <Pagination className="mx-0 w-auto justify-start lg:justify-end">
                <PaginationContent>
                  <PaginationItem><PaginationPrevious disabled={visiblePage === 1} onClick={() => setCurrentPage(Math.max(1, visiblePage - 1))} /></PaginationItem>
                  {paginationItems(visiblePage, totalPages).map((item) => (
                    <PaginationItem key={item}>
                      {typeof item === "number" ? (
                        <PaginationLink isActive={item === visiblePage} aria-label={`Go to page ${item}`} onClick={() => setCurrentPage(item)}>{item}</PaginationLink>
                      ) : <PaginationEllipsis />}
                    </PaginationItem>
                  ))}
                  <PaginationItem><PaginationNext disabled={visiblePage === totalPages} onClick={() => setCurrentPage(Math.min(totalPages, visiblePage + 1))} /></PaginationItem>
                </PaginationContent>
              </Pagination>
            </div>
          )}
      ...
      <CurrencyDetailSheet currency={selectedCurrency} open={detailOpen} onOpenChange={setDetailOpen} onAction={(currency) => setPendingAction({ currency, action: currency.active ? "deactivate" : "activate" })} />
      <CurrencyActionDialog action={pendingAction} open={!!pendingAction} onOpenChange={(open) => !open && setPendingAction(null)} onConfirm={confirmAction} isLoading={isActivating || isDeactivating} />
    </div>
  );
}
