...
export default function EmailTemplatePreview() {
  ...
  const activeCategoryName = isEnglish
    ? activeConfig?.nameEn || emailPreviewCategory
    : activeConfig?.nameKh || emailPreviewCategory;
  ...
  const handleSendTestEmail = async () => {
    ...
    if (isEnglish) {
      switch (emailPreviewCategory) {
        case "DAILY_EXPENSE":
          title = "Daily Expense Reminder: Don't forget to track today's expenses";
          message = "Please record your daily transactions regularly to maintain clear budget tracking.";
          break;
        case "BUDGET_WARNING":
          title = "Budget Warning: 'Food & Dining' category is approaching limit";
          message = "You have spent 85% of your monthly food and dining budget.";
          amount = 340;
          break;
        case "SAVINGS_GOAL":
          title = "Savings Goal Reminder: 'Emergency Fund' reached 75%";
          message = "Congratulations! You are close to reaching your $1,000 savings goal.";
          amount = 750;
          break;
        case "RECURRING_TX":
          title = "Payment Reminder: Internet service bill of $35 is due tomorrow";
          message = "Please check your account balance for automatic billing tomorrow.";
          amount = 35;
          break;
        case "MONTHLY_SUMMARY":
          title = "July 2026 Financial Summary Report";
          message = "Total income $2,450, total expenses $1,280, net savings $1,170 report is ready.";
          amount = 1170;
          break;
      }
    } else {
    ...
    <div className="space-y-6 font-google-sans">
      ...
      {testSentToast && (
        <div className="flex items-center gap-3 rounded-2xl bg-amber-500/10 p-4 text-sm font-bold text-amber-700 dark:text-[#FFC83D] border border-[#FFC83D]/30 animate-in fade-in">
          ...
          <span>{dict.notifications.testEmailSentSuccess}</span>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-3xl bg-white p-5 shadow-sm dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800">
        <div className="space-y-1">
          <h2 className="text-base font-bold text-slate-800 dark:text-white font-google-sans">
            {dict.notifications.emailPreviewTitle}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {dict.notifications.emailPreviewDesc}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center min-w-[165px] h-10 gap-2 rounded-2xl bg-[#003377] px-4 py-2.5 text-xs font-bold text-white shadow-md hover:bg-[#002255] transition active:scale-95 disabled:opacity-50 dark:bg-[#FFC83D] dark:text-[#003377] whitespace-nowrap shrink-0"
          >
            <Send size={14} />
            <span>{dict.notifications.sendTestEmail}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {Object.values(CATEGORY_CONFIGS).map((cat) => (
          <button
            ...
            className={`min-h-[58px] flex flex-col items-center justify-center p-3 rounded-2xl text-xs font-bold transition border ${
          ...
          >
            <span className="text-center font-google-sans leading-snug line-clamp-2">
        ...
        <div className="flex items-center justify-between border-b border-slate-300 bg-slate-200/60 px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
              {dict.notifications.emailClientTitle}
            </span>
          </div>
          <div className="text-xs text-slate-400 font-mono">
            {dict.notifications.emailSentTo}
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xl font-bold tracking-tight text-white font-sans">
                    iStash <span className="text-[#FFC83D]">{dict.notifications.emailBannerNotification}</span>
                  </span>
                </div>
<<<<<<< HEAD
...
>>>>>>> 17cb3ce3e288d4fd37c9f2ea926c41fd3cc16c0f
            ...
            <div className="p-6 sm:p-8 space-y-6 font-google-sans">
              <div className="border-b pb-4 border-slate-200 dark:border-slate-800">
                <p className="text-xs text-slate-400">{dict.notifications.emailSubjectLabel}</p>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white mt-1">
                  [{activeCategoryName}] {dict.notifications.emailSubjectSuffix}
                </h2>
              </div>

              {/* Greeting */}
              <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                {dict.notifications.emailGreeting}
              </p>
                    ...
                    <div className="flex items-center gap-2 text-[#003377] dark:text-[#FFC83D] font-bold text-sm">
                      ...
                      <span>
                        {isEnglish
                          ...
                          : "ដល់ម៉ោងកត់ត្រាការចំណាយប្រចាំថ្ងៃរបស់អ្នកហើយ!"}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                      {isEnglish
                        ? "Please remember to record your income and expenses for today. Regular tracking helps you stay within budget and avoid overspending."
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl bg-red-500/10 p-5 border border-red-500/30 space-y-3">
                    <div className="flex items-center gap-2 text-red-600 dark:text-red-400 font-bold text-sm">
                      ...
                      <span>
                        {isEnglish
                          ? "Budget Limit Warning (85% reached)"
                          : "ការព្រមានអំពីការចំណាយលើសពីដែនកំណត់ថវិកា (85%)"}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                      {isEnglish
                        ? "Your 'Food & Dining' expenses have reached $340.00 of your allocated $400.00 budget for this month."
                    ...
                    <div className="space-y-1 pt-1">
                      <div className="flex justify-between text-xs font-bold text-slate-800 dark:text-slate-200">
                        <span>{isEnglish ? "Warning: 85%" : "ប្រកាសអាសន្ន៖ 85%"}</span>
                        <span>$340 / $400</span>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl bg-emerald-500/10 p-5 border border-emerald-500/30 space-y-3">
                    <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-bold text-sm">
                      ...
                      <span>
                        {isEnglish
                          ? "Congratulations! Savings goal reached 75%"
                          : "អបអរសាទរ! គោលដៅសន្សំប្រាក់សម្រេចបាន 75%"}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                      {isEnglish
                        ? "You have saved $750.00 towards your 'Emergency Fund' goal. You only need $250.00 more to complete your $1,000.00 goal!"
                  ...
                  <div className="rounded-2xl bg-[#003377]/10 dark:bg-slate-800/70 p-5 border border-[#003377]/20 dark:border-slate-700/80 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                      <div className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-800">
                        <span className="text-slate-400">{isEnglish ? "Bill:" : "វិក្កយបត្រ៖"}</span>
                        <p className="font-bold text-slate-800 dark:text-white">
                          {isEnglish ? "Internet Service" : "សេវាអ៊ីនធឺណិត"}
                        </p>
                      </div>
                      <div className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-800">
                        <span className="text-slate-400">{isEnglish ? "Amount:" : "ចំនួនទឹកប្រាក់៖"}</span>
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl bg-indigo-500/10 p-5 border border-indigo-500/30 space-y-3">
                    <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-bold text-sm">
                      ...
                      <span>
                        {isEnglish
                          ? "July 2026 Financial Summary"
                          : "សេចក្តីសង្ខេបហិរញ្ញវត្ថុខែកក្កដា ២០២៦"}
                      </span>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-center text-xs">
                      <div className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-800">
                        <span className="text-slate-400">{isEnglish ? "Income" : "ចំណូល"}</span>
                        <p className="font-bold text-emerald-600 dark:text-emerald-400">$2,450</p>
                      </div>
                      <div className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200/80 dark:border-slate-800">
                        <span className="text-slate-400">{isEnglish ? "Expense" : "ចំណាយ"}</span>
                      ...
                      <div className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-800">
                        <span className="text-slate-400">{isEnglish ? "Savings" : "សន្សំ"}</span>
...
=======
              <div className="border-t pt-4 border-slate-200 dark:border-slate-800 text-[11px] text-slate-400 text-center space-y-1">
                <p>{dict.notifications.emailFooterCopyright}</p>
                <p>{dict.notifications.emailFooterUnsub}</p>
>>>>>>> 17cb3ce3e288d4fd37c9f2ea926c41fd3cc16c0f
              </div>
            </div>
          </div>
