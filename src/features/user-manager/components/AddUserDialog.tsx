...
export default function AddUserDialog({
  ...
  function validate(): boolean {
    ...
    if (form.temporaryPassword.length < 8)
      next.temporaryPassword = "Must be at least 8 characters.";
      ...
      >
        <DialogHeader className="relative mb-0 border-b border-border/70 bg-gradient-to-r from-[#003377]/8 via-background to-[#FEDB55]/12 px-6 py-6 pr-14 sm:px-8">
          <div className="flex items-start gap-4 text-left">
            ...
            <div>
              <DialogTitle className="text-2xl text-[#003377] dark:text-[#FEDB55]">
                {t("Add New User")}
              </DialogTitle>
              <DialogDescription className="mt-1">
                {t("Create a new iStash account.")}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        >
          <div className="space-y-7 px-6 py-6 sm:px-8">
            <section className="space-y-4">
              <div className="flex items-center gap-3">
                <span className="grid size-9 place-items-center rounded-xl bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                ...
                <div>
                  ...
                  <p className="text-base text-muted-foreground">
                    {t("Enter the user's basic details and access role.")}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                >
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="h-12 rounded-xl bg-background pl-12 pr-4 text-base"
                    />
                  </div>
                </Field>
                <Field label={t("Role")} htmlFor="role" required>
                  <Select
                    ...
                    value={form.role}
                    onValueChange={(value) =>
                      update("role", value as FormState["role"])
                    }
                  >
                    <SelectTrigger
                      id="role"
              ...
              {form.role === "ADMIN" && (
                <div className="flex items-start gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-base text-blue-800 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-200">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                >
                  <PasswordInput
                    ...
                    invalid={Boolean(errors.temporaryPassword)}
                    showLabel={t("Show temporary password")}
                    hideLabel={t("Hide temporary password")}
                  />
                </Field>
                <Field
                ...
                >
                  <PasswordInput
                    ...
                    showLabel={t("Show confirmed password")}
                    hideLabel={t("Hide confirmed password")}
                  />
                </Field>
              </div>
            </section>
          ...
          <DialogClose className="sticky bottom-0 mt-0 border-t border-border/70 bg-card/95 px-6 py-4 backdrop-blur sm:px-8">
            <Button
              type="button"
              variant="outline"
              className="h-11 min-w-28 rounded-xl text-base"
              onClick={() => {
                reset();
                onOpenChange(false);
              }}
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="h-11 min-w-44 rounded-xl bg-[#FEDB55] px-5 text-base font-semibold text-[#003377] shadow-sm hover:bg-[#f0ba33]"
            >
              {isLoading ? (
  ...
  return (
    <div className="relative">
      <Input
        ...
        className="h-12 rounded-xl bg-background px-4 pr-12 text-base"
      />
      <button
        type="button"
        onClick={onToggle}
        className="absolute right-2 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-lg text-muted-foreground transition hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
