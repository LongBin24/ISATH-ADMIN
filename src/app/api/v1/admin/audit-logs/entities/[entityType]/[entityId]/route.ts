...
export async function GET(
  ...
  let filtered = mockAuditLogs.filter(
    (item) =>
      item.entityType.toUpperCase() ===
        decodeURIComponent(entityType).toUpperCase() &&
      item.entityId.toLowerCase() === decodeURIComponent(entityId).toLowerCase()
  );
  ...
  const content = totalElements > 0 ? filtered.slice(start, start + size) : [];
