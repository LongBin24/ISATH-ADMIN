  ...
  import { LayoutDashboard, Tags, Globe, Bot, MessageSquare, Bell, UserCog, X, ShieldAlert, LogOut } from "lucide-react";
  ...
  export default function Sidebar({ isOpen = true, isCollapsed = false, onClose }: SidebarProps) {
          ...
          <nav className="flex flex-1 flex-col gap-2">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon size={20} />
                  <span className="text-base font-medium leading-6">{t(item.label)}</span>
                </Link>
              );
            })}
          </nav>
